...
class LitellmChatModel:
    ...
    async def achat(
        ...
        # 获取原始内容，如果为None则设为空字符串
        raw_content = response.choices[0].message.content or ""

        messages.append({
            "role": "assistant",
            "content": raw_content,  # type: ignore
        })
        ...
        if "response_format" in new_kwargs:
            ...
            try:
                # 1. 尝试直接解析 (处理空字符串或者是纯空白的情况)
                to_parse = raw_content if raw_content.strip() else "{}"
                parsed_dict: dict[str, Any] = json.loads(to_parse)
            
            except json.JSONDecodeError as e:
                # 2. 解析失败，打印原始内容以便调试
                logger.warning(f"JSONDecodeError occurred. Raw content length: {len(raw_content)}")
                logger.warning(f"Raw content head: {raw_content[:200]!r}...") # 只打印前200字符防止刷屏
                
                # 3. 尝试使用正则提取 Markdown 中的 JSON (```json ... ```) 或者纯文本中的JSON对象
                try:
                    # 寻找第一个 { 和 最后一个 } 之间的内容
                    ...
                    if match:
                        ...
                        logger.info("Successfully repaired JSON using regex.")
                    else:
            ...
            if inspect.isclass(new_kwargs["response_format"]) and issubclass(
                ...
                # 如果解析为空字典且模型有必需字段，这里可能会再次报错，但那是Validation Error了
                try:
